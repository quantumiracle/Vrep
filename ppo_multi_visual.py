...
S_DIM, A_DIM, CHANNEL = 64, 6, 3       # state and action dimension
...
class Worker(object):
    def __init__(self, wid):
        self.wid = wid
        dt = 100e-3
        r = vrep_sawyer.VrepSawyer(dt, port_num = int(19990+wid))
        self.env= simulator.Simulator(r,dt,target_x=0,target_y=0,target_z=0,visualize=False)
        self.ppo = GLOBAL_PPO
    ...
    def work(self):
        ...
        while not COORD.should_stop():
            self.env.reset()
            target_x,target_y,target_z = self.env.randomly_place_target()
            vs, s = self.env.get_robot_state()  # fisrt dim is visual, second dim is numerical
            step+=1
            ep_r = 0
            ...
            for t in range(EP_LEN):
                ...
                a = self.ppo.choose_action(vs)
                # print('a: ', a)
                self.env.set_control(a)
                self.env.step()
                vs_, s_ = self.env.get_robot_state()
                vs_ = np.float32(vs_)/256
                r,done = self.env.get_reward_and_done(s_)
                ...
                vs=vs.reshape(-1)  # convert from 3D to 1D
                buffer_s.append(vs)
                buffer_a.append(a)
            ...
            if step % 10==0:  # plot every N episode; some error about main thread for plotting
                ...
                try:
                    plt.savefig('./ppo_multi_visual.png')
                except:
...
if __name__ == '__main__':
    model_path = './model/ppo_multi_visual'
    if args.train:
        GLOBAL_PPO = PPO()
